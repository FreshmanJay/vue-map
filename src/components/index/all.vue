            ...
            <div class="item_left">
                ...
                <div class="item_infor">
                    <div class="item_top">
                        ...
                        <div class="item_mid"  v-if="item.marketingActivityList.length>1" v-for="obj in item.marketingActivityList">
                            <span  v-if="obj.activityType==2">{{obj.activityPrivilege}}</span>
                        </div>
                        <div class="item_mid" v-else>
                            <span>{{obj.activityPrivilege}}</span>
                        </div>
                        <div class="item_bot">
...
<script type="text/ecmascript-6">
    ...
    export default {
            ...
            methods:{
                ...
                skip(index){
                     var activityId;
                    if(this.list[index].marketingActivityList.length>1){
                        for(var i in this.list[index].marketingActivityList){
                            if(this.list[index].marketingActivityList[i].activityType==2){
                                 activityId=this.list[index].marketingActivityList[i].activityId

                            }
                        }
                    }else{
                         activityId=this.list[index].marketingActivityList[0].activityId
